...
void free_resources(pthread_t *thread_id, argsForThread *thread_args) {
    free(thread_args);
    free(thread_id);
}
...
int calc_pi(int num_of_threads, double *res) {
    ...
    argsForThread *thread_args = (argsForThread *)malloc(num_of_threads * sizeof(argsForThread));
